...
//          noop(); // keep JSLint happy
		...
		if(!nodeTitle){
			...
			if( opts.noLink || data.noLink ) {
				if(data.parent){
					nodeTitle = '<a href="' + href + '" > <i class=" '+data.icone+' "></i><span>' + data.title + '</span></a>';
				}else{
...
//              this.tree.logDebug("nodeTitle: " + nodeTitle);
				...
				// upadte for context nav
				context = "", loadWith="ajax";
				if(data.context)
					context = 'data-toggle="context" data-target="'+data.context_target+'"';
				if(data.load)
					loadWith = data.load;
				idTab = (data.tab)? data.tab : "tab-"+$.now() ;
				iconeTab = (data.iconeTab)? data.iconeTab : '';
				if(data.parent){
					nodeTitle = '<a href="' + href + '" data-iconeTab="'+iconeTab+'" > <i class=" '+data.icone+' "></i><span>' + data.title + '</span></a>';
				}else{
					nodeTitle = '<a href="' + href + '" data-iconeTab="'+iconeTab+'" ' + context + ' class=" item-tree ' + data.status + ' ' + opts.classNames.title + '"' + tooltip + '>' + data.title + '</a>';
				}
			}
		}
		res += nodeTitle;
...
//              this.span.innerHTML = this._getInnerHtml();
			...
			this.span.className = cnList.join(" ") + " " + data.status;
