...
var melisDashBoardDragnDrop = {
	...
	cacheDom: function() {
		...
		this.$dbMsg = this.$body.find(
			"#" + activeTabId + " " + this.melisDashboardMsg
		);

		// plugin sidebar
		this.$dashPsBox = $(".melis-core-dashboard-ps-box");
		this.$dashPluginBtn = this.$dashPsBox.find(
			".melis-core-dashboard-filter-btn"
		);
		this.$dashSnipsBox = this.$dashPsBox.find(
	...
	setAdjustGridMeasurements: function() {
		...
		self.$activeGS.attr(
			"data-min-width",
			self.$activeGS.outerWidth() - self.$melisLeftMenu.outerWidth()
		);
	...
	dragWidget: function() {
		...
		$(
			".melis-core-dashboard-plugin-filter-box .melis-core-dashboard-plugin-snippets"
		).draggable({
			helper: "clone",
			revert: "invalid",
			appendTo: "body",
			start: function(event, ui) {
				$(ui.helper)
					.find(".melis-plugin-tooltip")
					.hide();
			},
			drag: function(event, ui) {
				var $gridPH = $(
					"#" + activeTabId + " .tab-pane .grid-stack .grid-stack-placeholder"
				);
				 ...
				 */
				var pluginCount = melisDashBoardDragnDrop.$gs.find("div[data-gs-id]")
						.length,
					dashboardMsg = $("#" + activeTabId).find(
						melisDashBoardDragnDrop.melisDashboardMsg
					),
				...
				if (
					pluginCount === 0 &&
					!dragArea.hasClass("melis-core-dashboard-plugin-snippets")
				) {
	...
	dropWidget: function(widget) {
		...
		self.$gs.droppable({
			...
			drop: function(event, ui) {
				...
				var pluginMenu = $(ui.helper[0])
					.find(".plugin-json-config")
					.text();
	...
	addWidget: function(dataString) {
		...
		var $mcDashPlugSnippets = $(
			"#" +
				activeTabId +
				" .tab-pane .grid-stack .melis-core-dashboard-plugin-snippets"
		);
		$mcDashPlugSnippets.attr("data-gs-width", 6);
		$mcDashPlugSnippets.attr("data-gs-height", 3);

		var mcLoader =
			"<div class='overlay-loader'><img class='loader-icon spinning-cog' src='/MelisCore/assets/images/cog12.svg' alt=''></div>";
		...
		var request = $.post(
			"/melis/MelisCore/DashboardPlugins/getPlugin",
			dataString
		);
	...
	serializeWidgetMap: function(items) {
		...
		$.each(items, function(key, value) {
			var dataTxt = $(value)
				.find(".dashboard-plugin-json-config")
				.text();
	...
	saveDBWidgets: function(dataString) {
		...
		var saveDashboardLists = $.post(
			"/melis/MelisCore/DashboardPlugins/saveDashboardPlugins",
			dataString
		);
	},
			...
			if ($melisLeftMenu.hasClass("shown")) {
				if ($pluginBox.hasClass("shown")) {
					...
					setTimeout(function() {
						$activeGS.attr(
							"data-min-width",
							$activeGS.outerWidth() - pluginBoxWidth
						);
				...
				// left sidebar menu and dashboard plugin menu not shown
					...
					setTimeout(function() {
						$activeGS.attr(
							"data-min-width",
							$activeGS.outerWidth() + melisLeftMenuWidth
						);
						$activeGS.attr("data-max-width", $activeGS.outerWidth());
					}, 500);
				}
	...
	resizeStopWidget: function() {
		...
		self.$gs.on("gsresizestop", function(event, elem) {
			var $elem = $(elem),
				...
				// latest comments
				$cFilters = $elem.find(
					".melis-cms-comments-dashboard-latest-comments .mccom-filters-tab .row .mccom-filter"
				),
				$sCont = $cFilters.find(".form-group .select2-container"),
				$profileImg = $elem.find(
					".melis-cms-comments-dashboard-latest-comments .column-comment-profile-img"
				);
	...
	latestCommentsPluginUIRes: function() {
		var $com = $("#" + activeTabId + " .grid-stack .grid-stack-item").find(
		...
		if ($items.length !== 0) {
			...
			melisCoreTool.confirm(
				...
				function() {
					...
					// Show empty-dashboard message
					var dashboardMsg = $("#" + activeTabId).find(
						melisDashBoardDragnDrop.melisDashboardMsg
					);
			...
			$(
				"#" +
					activeTabId +
					" .grid-stack .grid-stack-item .dashboard-plugin-delete"
			).each(function(i, v) {
				var $this = $(this);
	...
	refreshWidget: function(el) {
		...
		if (dataTxt) {
			...
			request.done(function(data) {
				...
				// place in the last location
				$(widget).insertBefore(
					$("div").find("[data-gs-id='" + nextElementId + "']")
				);
				// assigning current plugin
				self.setCurrentPlugin(widget);
	...
	checkDashboardElemWidths: function() {
		...
		$bubblePlugin.attr(
			"data-min-width",
			$bubblePlugin.outerWidth() - $pluginBox.outerWidth()
		);
		...
		if ($bubblePlugin.length) {
			if ($dbMsg.length && $dbMsg.is(":visible")) {
				if (
					$dbMsg.closest(".melis-dashboard-plugins").find(".pt-0").length > 0
				) {
