...
var melisCore = (function(window) {
		...
		$.ajax({
			...
			.done(function(data) {
				...
				if (data.flashMessage.length) {
					...
					var tempData = "";
					var clearData =
						"<li style='border-left: 0 solid #ce5459;'><button id='clearNotifBtn' class='btn btn-primary' style='width:100%; border-width:0'>" +
						translations.tr_meliscore_clear_notifications +
						"</button></li>";
					$.each(data, function(index, element) {
						$.each(element, function(index, fm) {
							tempData +=
								"" +
								"<li>" +
								"	<span class='rounded-circle media-object " +
								fm.image +
								"'></span>" +
								"   <div class='media'>" +
								"       <div class='media-body'>" +
								"           <a  class='strong text-primary'>" +
								fm.title +
								"</a><span class='time-email'>" +
								fm.date_trans +
								" " +
								fm.time +
								"</span>" +
					...
					$body
						.find(
							"#id_meliscore_header_flash_messenger.dropdown.notification a span.badge"
						)
						.text(ctr);
				}
			})
	...
	function resetUserRights() {
		...
		$.ajax({
			...
			.done(function(data) {
				if (data.success) {
					if (
						$("#user-name-link").attr("data-user-id") ===
						$("#edituserid").html()
					)
	...
	function openDashboard() {
		melisHelper.tabOpen(
			"Dashboard",
			"fa-dashboard",
			"id_meliscore_toolstree_section_dashboard",
			"meliscore_dashboard",
			{ dashboardId: "id_meliscore_toolstree_section_dashboard" },
			function() {
	...
	function tabMenuClick() {
		...
		if ($.browser) {
			...
			var iHeight;
			setTimeout(function() {
				iHeight = $("#" + activeTabId + " .melis-iframe")
					.contents()
					.find("html")
					.height();
			...
			var iHeight = $("#" + activeTabId + " .melis-iframe")
				.contents()
				.find("html")
				.height();
		...
		$("#" + activeTabId + " .page-head-container > .innerAll").removeAttr(
			"style"
		);
	...
	$body.on("click", ".multi-value-input .remove-tag", function() {
		...
		$(this)
			...
			.fadeOut(500, function() {
				// run it when the fadeOut completes the action
		...
		if (event.keyCode == 188) {
			if (commaHandler === false && tagValue && tagValue !== ",") {
				var newLi =
					"<li><span>" +
					tagValue +
					'</span><a class="remove-tag fa fa-times"></a></li>';
	...
	$body
		.on("shown.bs.tab", ".nav-tabs li a", navTabsSwitch)
		.filter(":first")
		.click();
	...
	$body.on(
		"shown.bs.tab",
		"#melis-id-nav-bar-tabs li a.tab-element",
		tabMenuClick
	);
	...
	function showPlugLists() {
		if ($(this).hasClass("active")) {
			...
			$(".melis-core-dashboard-filter-btn.active")
				.removeClass("active")
				.siblings(".melis-core-dashboard-plugin-snippets-box")
				.slideUp();
			$(this).addClass("active");
	...
	function loadCustomCheckboxElement() {
		...
		$.each($checkbox, function() {
			...
			$this
				.parent("div")
				.append("<label for=" + $id + " class='cls-checkbox-label'></label>");
		});
	}
	...
	$(window).on("resize", function() {
		...
		if (screenSize >= 768) {
			...
			$("#content, #id_meliscore_leftmenu, #id_meliscore_footer").removeAttr(
				"style"
			);
			...
			$("#id_meliscore_header .navbar-right > li").each(function(key, value) {
				$(this)
					.children("a")
					.append("<span class='title'>" + $(this).data("title") + "</span>");
	...
	$(".dashboard-workflow-container .nav-tabs li").height(
		$(".dashboard-workflow-container .nav-tabs").height()
	);

	// melis cms tools tree
	if ($("#meliscms_toolstree_section").length > 0) {
		$("#site-tree-cont")
			.prependTo("#meliscms_toolstree_section_tools")
			.removeClass("hidden")
			.show();
	}
