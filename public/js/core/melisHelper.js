var melisHelper = (function() {
	...
	function melisOkNotification(title, message, color) {
		...
		$.gritter.add({
			...
			time: 3000,
			image:
				"/melis/MelisCore/MelisAuth/getProfilePicture?v=" +
				new Date().getTime(),
		});
		//set the color
	...
	function melisMultiKoNotification(title, message, errors, closeByButtonOnly) {
		...
		var closeByButtonOnly =
			closeByButtonOnly !== true ? "overlay-hideonclick" : "";
		...
		if (!success) {
			$.each(errors, function(key, error) {
				if ("form" in error) {
					...
					$(selector + " .form-control[name='" + key + "']")
						.parents(".form-group")
						.find("label")
						.css("color", "red");
				}
		...
		if (targetInput.length) {
			targetInput.each(function() {
				...
				var switchBtn =
					'<label for="' +
					$(this).attr("name") +
					'" class="d-flex flex-row justify-content-between"><div class="label-text">' +
					$(this).data("label") +
					attribRequired +
					"</div>" +
					attribTooltip +
					"</label>" +
					'<div class="make-switch user-admin-switch" data-label-icon="glyphicon glyphicon-resize-horizontal" data-on-label="' +
	...
	function checkIfCmsPage() {
		var $melisCms = $body.find(
				"#" + activeTabId + ".tab-pane.container-level-a"
			),
	...
	function tabSwitch(tabID) {
		var $tabElement = $(
				"#melis-id-nav-bar-tabs a.tab-element[data-id='" + tabID + "']"
			),
			subMenu = $tabElement.closest(".nav-group-dropdown"),
			$navBarTabsLi = $("body #melis-id-nav-bar-tabs li"),
			$navBarTabsLiCont = $(
				"body #melis-id-nav-bar-tabs li, .container-level-a"
			),
	...
	function tabClose(ID, fromGroup) {
		...
		if (
			prevActiveTab == "id_meliscore_toolstree_section_dashboard" &&
			!nextActiveTab
		) {
			$("#close-all-tab").hide();
			$("#close-all-tab")
		...
		if (melisCore.screenSize <= 767) {
			...
			if ($navTabs.children("li").length === 0) {
				var empty =
					"<strong>(" + translations.tr_meliscore_empty + ")</strong>";
	...
	function tabOpen(
		...
		var alreadyOpen = $(
			"body #melis-id-nav-bar-tabs li a.tab-element[data-id='" + zoneId + "']"
		);
		...
		if (alreadyOpen.length < 1) {
			...
			li += title + "</span></a>";
			li +=
				"<a class='close close-tab' data-id='" +
				zoneId +
				"'>" +
				translations.tr_meliscore_notification_modal_Close +
				"</a>";
			...
			var div =
				...
				"' id='" +
				zoneId +
				"' class='tab-pane container-level-a'></div>";
	...
	function disableTab(tabId) {
		$("li a.tab-element[data-id='" + tabId + "']")
	...
	function melisInitDataTable(requiredSettings) {
		if (typeof requiredSettings === "object") {
			if (typeof requiredSettings.langTrans == "undefined")
				langTrans = "default";
