		...
		Dropzone.options.rfmDropzone = {
			...
			accept: function(file, done) {
				...
				if ($.inArray(extension, allowed_ext) > -1) {
					...
					done("<?php echo trans('File extension is not allowed.');?>");
				}
			}
		};
		if (image_editor) {
...
<body>
	...
	<input type="hidden" id="insert_folder_name" value="<?php echo trans('Insert folder name');?>" />
...
<div class="uploader">
    <div class="pull-left">
        <div class="text-center">
            &nbsp;<button class="btn btn-inverse close-uploader"><i class="icon-backward icon-white"></i> <?php echo trans('Back')?></button>
        </div>
    </div>
    <div class="clearfix">
    </div>
	...
	<div class="tabbable upload-tabbable"> <!-- Only required for left/right tabs -->
		<ul class="nav nav-tabs">
			<li class="active"><a href="#tab1" data-toggle="tab"><?php echo trans('Upload');?></a></li>
			<?php if($url_upload){ ?>
			<li><a href="#taburl" data-toggle="tab"><?php echo trans('Upload via URL');?></a></li>
			<?php } ?>
			<?php if($java_upload){ ?>
		...
		<div class="tab-content">
			<div class="tab-pane active" id="tab1">
				<form action="dialog.php" method="post" enctype="multipart/form-data" id="rfmDropzone" class="dropzone">
					...
					<div class="fallback">
						<h3><?php echo  trans('Upload file')?>:</h3><br/>
			...
			<div class="tab-pane" id="taburl">
				...
				<form class="form-horizontal">
					<div class="control-group">
						<label class="control-label" for="url"><?php echo trans('URL');?></label>
						<div class="controls">
							<input type="text" class="input-block-level" id="url" placeholder="<?php echo trans('URL');?>">
						</div>
					</div>
					<div class="control-group">
						<div class="controls">
							<button class="btn btn-primary" id="uploadURL"><?php echo  trans('Upload file');?></button>
						</div>
					</div>
				</form>
			</div>
	...
	<div class="navbar-inner">
		...
		<div class="filters">
			...
			<div class="span4 half">
				<?php if($upload_files){ ?>
				<button class="tip btn upload-btn" title="<?php echo  trans('Upload file');?>"><i class="rficon-upload"></i></button>
				<?php } ?>
				<?php if($create_text_files){ ?>
				<button class="tip btn create-file-btn" title="<?php echo  trans('New file');?>"><i class="icon-plus"></i><i class="icon-file"></i></button>
				<?php } ?>
				<?php if($create_folders){ ?>
				<button class="tip btn new-folder" title="<?php echo  trans('New folder')?>"><i class="icon-plus"></i><i class="icon-folder-open"></i></button>
				<?php } ?>
				<?php if($copy_cut_files || $copy_cut_dirs){ ?>
				<button class="tip btn paste-here-btn" title="<?php echo trans('Paste');?>"><i class="rficon-clipboard-apply"></i></button>
				<button class="tip btn clear-clipboard-btn" title="<?php echo trans('Clear clipboard');?>"><i class="rficon-clipboard-clear"></i></button>
				<?php } ?>
			</div>
			<div class="span2 half view-controller">
				<button class="btn tip<?php if($view==0) echo " btn-inverse";?>" id="view0" data-value="0" title="<?php echo trans('Thumbnails');?>"><i class="icon-th <?php if($view==0) echo "icon-white";?>"></i></button>
				<button class="btn tip<?php if($view==1) echo " btn-inverse";?>" id="view1" data-value="1" title="<?php echo trans('List');?>"><i class="icon-align-justify <?php if($view==1) echo "icon-white";?>"></i></button>
				<button class="btn tip<?php if($view==2) echo " btn-inverse";?>" id="view2" data-value="2" title="<?php echo trans('Column List');?>"><i class="icon-fire <?php if($view==2) echo "icon-white";?>"></i></button>
			</div>
			<div class="span6 entire types">
				...
				<label id="ff-item-type-5" title="<?php echo trans('Music');?>" for="select-type-5" class="tip btn ff-label-type-5"><i class="icon-music"></i></label>
					<?php } ?>
				<?php } ?>
				<input accesskey="f" type="text" class="filter-input <?php echo (($_GET['type']!=1 && $_GET['type']!=3) ? '' : 'filter-input-notype');?>" id="filter-input" name="filter" placeholder="<?php echo fix_strtolower(trans('Search'));?>..." value="<?php echo $filter;?>"/><?php if($n_files>$file_number_limit_js){ ?><label id="filter" class="btn"><i class="icon-play"></i></label><?php } ?>
...
<!-- header div end -->
	...
	<li class="pull-right"><a class="btn-small hidden" href="javascript:void('')" id="change_lang_btn"><i class="icon-globe"></i></a></li>
	<?php } ?>
	...
	<div class="span12">
		...
		foreach ($files as $file_array) {
			...
			if($ftp && $file!='..' && $file!=$new_name){
				//rename
				rename_folder($subdir.$file,$new_name,$ftp,$config);
				$file=$new_name;
			}
			...
			<?php }else{ ?>
					...
					<figcaption>
						...
						<a href="javascript:void('')" class="tip-left erase-button <?php if($delete_folders && !$file_prevent_delete) echo "delete-folder";?>" title="<?php echo trans('Erase')?>" data-confirm="<?php echo trans('Are you sure to delete the folder and all the elements in it?');?>" data-path="<?php echo $rfm_subfolder.$subdir.$file;?>" >
			...
			?>		<figure data-name="<?php echo $file ?>" data-type="<?php if($is_img){ echo "img"; }else{ echo "file"; } ?>">
				...
				<div class="img-precontainer">
					...
					<div class="img-container">
						<img class="<?php echo $show_original ? "original" : "" ?><?php echo $is_icon_thumb ? " icon" : "" ?><?php echo $lazy_loading_enabled ? " lazy-loaded" : ""?>" <?php echo $lazy_loading_enabled ? "data-original" : "src"?>="<?php echo $config['base_url'].'/'. $src_thumb;?>">
					</div>
				</div>
				<div class="img-precontainer-mini <?php if($is_img) echo 'original-thumb' ?>">
					...
					<div class="img-container-mini">
					<?php if($mini_src!=""){ ?>
					<img class="<?php echo $show_original_mini ? "original" : "" ?><?php echo $is_icon_thumb_mini ? " icon" : "" ?><?php echo $lazy_loading_enabled ? " lazy-loaded" : ""?>" <?php echo $lazy_loading_enabled ? "data-original" : "src"?>="<?php echo $config['base_url'].'/'. $src_thumb;?>">
					<?php } ?>
					</div>
				</div>
				...
				<figcaption>
					...
					<a href="javascript:void('')" class="tip-left erase-button <?php if($delete_files && !$file_prevent_delete) echo "delete-file";?>" title="<?php echo trans('Erase')?>" data-confirm="Are you sure you want to delete this file?" data-path="<?php echo $rfm_subfolder.$subdir.$file;?>">
...
<script>
    ...
    <script src="js/events.tinymce.js"></script>
</body>
</html>
