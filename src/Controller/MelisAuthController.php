...
class MelisAuthController extends MelisAbstractActionController
{
        ...
        if ($request->isPost()) {
            ...
            if ($loginForm->isValid()) {
                ...
                if (!empty($userData)) {
                    ...
                    // The legacy MD5 / mcrypt auto-transition code was removed: every stored
                    // password is bcrypt (password_hash / PASSWORD_DEFAULT). A stored hash in
                    // any other format simply fails verification and the user must reset via
                    // the "forgot password" flow.

                    // @var $password - user provided password (swapped for the stored hash on success)
                    $password = $postValues['usr_password'];
                    // @var $needReset - Flag if user needs to use the forgot password link
                    $needReset = false;
                    // @var $isPassExpired - Flag if user needs to renew password
                    $isPassExpired = false;
    ...
    protected function rememberMe($username, $password)
    {
        ...
        // (base64-encoded password) is actively expired here. cookie1/remember are HttpOnly and
        // Secure over HTTPS: both are read server-side only, never from JS.
        ...
        $clearPass = new SetCookie('cookie2', '', strtotime('-1 day'), null, null, $isSecure, true, null, null , SetCookie::SAME_SITE_STRICT );
