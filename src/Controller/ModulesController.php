...
class ModulesController extends AbstractActionController
{
    ...
    public function renderToolModulesContentAction()
    {
        ...
        $view->siteModules = $this->getModules(true);
        return $view;
    }
    ...
    public function getModules($useOnlySiteModule = false)
    {
        ...
        foreach($modules as $index => $modValues) {
            ...
            if(!in_array($modValues, $this->exclude_modules)) {
                $moduleLoadList = $moduleLoadFile;

                if(in_array($modValues, $moduleLoadList)) {
        ...
        if(!$useOnlySiteModule) {
            // displaying the modules the way they are being organized in the module loader
            foreach($this->getModuleSvc()->getModulePlugins() as $index => $modValues) {
                if(!in_array($modValues, $this->exclude_modules)) {
                    if(!in_array($modValues, $modules)) {
                        if($modValues != 'MySiteTest2')
                            $modulesList[$modValues] = 0;
                    }
                }
            }
