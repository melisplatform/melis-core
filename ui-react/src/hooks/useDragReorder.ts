...
/**
 * Touch-compatible drag-and-drop for the two-panel (Masquées/Visibles) column manager.
 *
 ...
 * to track the SAME gesture for both mouse and touch — one code path per input type, same
 * end-to-end reorder logic.
 *
 * Usage: each draggable row gets `data-col-item={col.id}` + `onMouseDown`/`onTouchStart` wired
 * to `startDrag(col.id)` + `style={{ touchAction: 'none' }}` (required so the browser doesn't
 * hijack the gesture as a page scroll before our JS sees it); each panel container gets
 * `data-col-panel="hidden"` or `"visible"`. Hit-testing during the drag uses
...
export function useDragReorder({ cols, onChange }: { cols: ColDef[]; onChange: (cols: ColDef[]) => void }) {
  ...
  // Handlers are created fresh per drag gesture (in `startDrag`) and stashed here so `endDrag`
  // removes the EXACT function references that were added — a mismatched reference would
  // silently fail to remove the listener and leak it.
